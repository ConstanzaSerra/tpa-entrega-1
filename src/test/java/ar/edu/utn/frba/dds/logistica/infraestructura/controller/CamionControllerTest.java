...
class CamionControllerTest {
    ...
    // No se puede crear un camion con patente vacia; el dominio lo rechaza con 400
    @Test
    void testCreate_PatenteVacia_Devuelve400() {
        CamionDTO inputDto = new CamionDTO(null, "", 5, 2, 5000);
        ...
        verify(camionRepository, never()).guardar(any());
    }

    // No se puede crear un camion con volumen cero o negativo
    @Test
    void testCreate_VolumenCero_Devuelve400() {
        CamionDTO inputDto = new CamionDTO(null, "ABC123", 0, 2, 5000);
        when(ctx.bodyAsClass(CamionDTO.class)).thenReturn(inputDto);
        when(ctx.status(HttpStatus.BAD_REQUEST)).thenReturn(ctx);
    ...
    // Caso feliz: crear un camion valido devuelve 201 con el recurso creado
    @Test
    void testCreate() {
